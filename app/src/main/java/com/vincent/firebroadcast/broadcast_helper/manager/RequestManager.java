...
import com.vincent.firebroadcast.broadcast_helper.data.KeyData;
import com.vincent.firebroadcast.broadcast_helper.data.Device;
import com.vincent.firebroadcast.broadcast_helper.data.FirebaseUser;
import com.vincent.firebroadcast.network_helper.MyOkHttp;
        ...
        databaseRefUsers.child(FirebaseUser.DATABASE_USERS).child(id).addListenerForSingleValueEvent(new ValueEventListener() {
            ...
            public void onDataChange(DataSnapshot dataSnapshot) {
                ...
                for (Device device : user.getDeviceList()) {
                    if (device.getToken().equals(device.getToken())) {
                        //已經註冊過該token
                        return;
                    }

                    if (device.getDevice() != null && device.getDevice().equals("android")) {
                        //有註冊過相同作業系統
                        return;
                    }
                }
